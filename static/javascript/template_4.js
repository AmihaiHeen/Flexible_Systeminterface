...
function createDesc(plc,b){
  if(descBool){
    ...
    descbox.setAttribute('class', 'accordition col-4 col-sm-6 mt-3 w-50 p-3 ');
    ...
    descbtn.setAttribute('class', 'btn btn-secondary  w-50 p-3');
...
function createRes(plc,b){
  if(resBool){
    ...
    resbox.setAttribute('class', 'accordition col-8 col-sm-6 mt-3');
    ...
    resbtn.setAttribute('class', 'btn btn-secondary');
    ...
    for(i=0;i<resIndex.length;i++){
      ...
      var rLiTxt = document.createTextNode('Result '+count+' at index: '+resIndex[i]);
      rLi.appendChild(rLiTxt);
      resUl.appendChild(rLi);
    }
